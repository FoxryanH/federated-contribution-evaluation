...
def optimize_smartfl_weights_with_analysis(
    ...
    grad_damping,
    grad_stability_mean_threshold,
    grad_stability_range_threshold,
    device,
):
    ...
    for step_idx in range(server_opt_steps):
        ...
        with torch.no_grad():
            ...
            if (
                mean_abs_grad < float(grad_stability_mean_threshold)
                and grad_range < float(grad_stability_range_threshold)
            ):
                steps_used = step_idx
                early_stopped = True
                ...
                print(
                    ...
                    "mean_abs_grad and grad_range both fell below the Yahoo Answers thresholds. "
                    f"mean_abs_grad={mean_abs_grad:.6f}, "
                    f"grad_range={grad_range:.6f}, "
                    f"mean_threshold={float(grad_stability_mean_threshold):.6f}, "
                    f"range_threshold={float(grad_stability_range_threshold):.6f}"
                )
                break
    ...
    with torch.no_grad():
        proxy_loss = base.smartfl_base.evaluate_proxy_loss_for_alpha(model, local_states, alpha, proxy_batches, device)
...
def compute_leave_one_out_proxy_metrics(model, local_states, analysis_alpha, proxy_batches, device):
    ...
    for client_idx in range(alpha_tensor.numel()):
        ...
        with torch.no_grad():
            loo_loss = float(
                base.smartfl_base.evaluate_proxy_loss_for_alpha(model, local_states, loo_alpha, proxy_batches, device)
                .item()
            )
...
def run_round_on_device_with_analysis(
    ...
    smartfl_result = optimize_smartfl_weights_with_analysis(
        ...
        args.grad_damping,
        args.grad_stability_mean_threshold,
        args.grad_stability_range_threshold,
        device,
    )
...
def print_setup_banner(code_version, partition_name, args, private_indices, task_indices, gen_test_indices, device, startup_tokens):
    print(
        ...
        f"labels={base.YAHOO_ANSWERS_LABELS}",
...
def run_hist_experiment(args, code_version, partition_name, output_json, setup_fn):
    ...
    full_dataset, full_targets = base.load_yahooanswers(args)
    private_indices, task_indices, gen_test_indices, split_metadata = base.balanced_three_way_split_indices(
        full_targets,
    ...
    global_model = base.build_model(args).to(device)
    round_records = []
    ...
    for round_idx in range(1, args.rounds + 1):
        ...
        trial_model = base.build_model(args).to(device)
    ...
    payload = {
        ...
        "server_proxy_batch_size": server_proxy_batch_size,
        "server_proxy_batches_per_round": server_proxy_batches_per_round,

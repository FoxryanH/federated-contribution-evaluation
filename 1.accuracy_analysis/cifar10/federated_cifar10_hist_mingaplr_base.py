...
def optimize_smartfl_weights_with_analysis(
    ...
    del server_opt_weight_tol  # preserved in signature for compatibility with existing callers
    ...
    for step_idx in range(server_opt_steps):
        ...
        proxy_loss_value, proxy_grad = base.compute_proxy_loss_and_grad_for_alpha(
        ...
        with torch.no_grad():
            ...
            next_alpha = base.normalize_raw_weights(raw)
    ...
    with torch.no_grad():
        proxy_loss = base.evaluate_proxy_loss_for_alpha(model, local_states, alpha, proxy_batches, device)
...
def compute_leave_one_out_proxy_metrics(model, local_states, analysis_alpha, proxy_batches, device):
    ...
    with torch.no_grad():
        full_proxy_loss = float(
            base.evaluate_proxy_loss_for_alpha(model, local_states, alpha_tensor, proxy_batches, device).item()
        )
    ...
    for client_idx in range(alpha_tensor.numel()):
        ...
        loo_alpha = base.normalize_raw_weights(loo_alpha)
        with torch.no_grad():
            loo_loss = float(
                base.evaluate_proxy_loss_for_alpha(model, local_states, loo_alpha, proxy_batches, device).item()
            )
...
def run_round_on_device_with_analysis(
    ...
    global_state = base.clone_state_dict_to_cpu(global_model.state_dict())
...
def build_window_average_weights(round_records, window_size):
    ...
    for offset in range(0, len(round_records), window_size):
        ...
        category_stats = {
            category: {"weight_sum": 0.0, "selected_count": 0}
        ...
        client_average_weights.append(
            {
                ...
                **client_metadata[client_id],
                ...
                "average_weight_when_selected": (
                    weight_sum / selected_count if selected_count > 0 else None
                ),
                "average_weight_over_all_rounds": (
                    weight_sum / total_rounds if total_rounds > 0 else None
                ),
            }
        )
...
def run_hist_experiment(
    ...
    output_path = base.resolve_output_path(output_json)

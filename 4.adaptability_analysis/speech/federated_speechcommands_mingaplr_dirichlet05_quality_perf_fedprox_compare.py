...
CODE_VERSION = "speechcommands_mingaplr_dirichlet05_quality_perf_fedprox_compare_v1"
DEFAULT_OUTPUT_JSON = Path("outputs/speechcommands_mingaplr_dirichlet05_quality_perf_fedprox_compare/results1.json")
...
def client_local_train_fedprox(global_state, dataset_loader, sample_count, device, args, local_seed):
    ...
    optimizer = base.smartfl_base.build_optimizer(local_model, args)
    ...
    local_state = base.smartfl_base.clone_state_dict_to_cpu(local_model.state_dict())
    del optimizer
    del criterion
    del local_model
    base.smartfl_base.clear_device_cache(device)
...
def run_experiment(args):
    ...
    print(f"Speech Commands quality FedProx compare JSON saved to: {output_path}")
